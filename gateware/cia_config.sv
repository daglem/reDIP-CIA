...
// Override four-state semantics of conditional operator.
`ifdef __ICARUS__
  `define bit(v) bit'(v)
`else
  `define bit(v) (v)
...
module cia_config (
    ...
    //
    //   CFG0 - Report configured CIA model in ICR bits 6:5
    //   CFG1 - Configure MOS 6526
    //   CFG2 - Configure MOS 8521
    //   CFG3 - Configure MOS 8520
    //   CFG5 - Report number of flash erase/program cycles since last reset
    ...
    typedef struct packed {
        ...
        logic [1:0]  wear;    // Flash erase / program cycles since last reset.
    } vars_t;
    ...
    always_ff @(posedge clk) begin
        // We use synchronous reset with a 24MHz clock, which has a period of
        // 1/24MHz = 41.67ns.
        // The longest required /CS deselect time between (possibly aborted)
        // operations in the flash chip is 30ns, i.e. the flash chip should
        // be able to recover from any reset signal bounce.
        if (res) begin
        ...
        unique0 case (current.state)
                  ...
                  unique case (current.cmdlen)
                    ...
                    'd3: begin                               // CFG n
                        unique0 case (chr)
                          ...
                          'd7: if (current.wear != '1) begin // CFG 7
                              // Allow max 3 erase / program cycles per reset;
                              // this makes it impossible for a malicious program
                              // to wear out the flash 4K block.
                              next.state  = WRITE;
                              next.wear   = current.wear + 1'd1;
                              spi_start   = '1;
                          end
                        endcase
                    end
          ...
          READ: begin
              if (~spi_busy) begin
                  next.state = PROMPT;
                  if (current.rstat)
                      next.icr65 = spi_model;
                  else
                      next.model = spi_model;
              end
          end
          WRITE: begin
              if (~spi_busy)
